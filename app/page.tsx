...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white selection:bg-blue-100 font-sans">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            <button onClick={() => scrollToSection('election-journey')} className="text-sm font-bold text-slate-600 hover:text-blue-600 transition-colors uppercase tracking-widest">Journey</button>
            <button onClick={() => scrollToSection('myth-vs-fact')} className="text-sm font-bold text-slate-600 hover:text-blue-600 transition-colors uppercase tracking-widest">Myths</button>
            <button onClick={() => scrollToSection('readiness')} className="text-sm font-bold text-slate-600 hover:text-blue-600 transition-colors uppercase tracking-widest">Readiness</button>
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="lg:hidden fixed inset-0 top-[72px] bg-white z-40 p-6 animate-in slide-in-from-top-4 duration-300">
             <div className="flex flex-col gap-6">
                <button onClick={() => scrollToSection('election-journey')} className="text-2xl font-black text-slate-900 text-left">Election Journey</button>
                <button onClick={() => scrollToSection('myth-vs-fact')} className="text-2xl font-black text-slate-900 text-left">Myth vs Fact</button>
                <button onClick={() => scrollToSection('readiness')} className="text-2xl font-black text-slate-900 text-left">Readiness Score</button>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-4 text-slate-400 font-bold text-sm">
              <li><a href="https://voters.eci.gov.in/" target="_blank" className="hover:text-white transition-colors">Voter Service Portal</a></li>
              <li><a href="https://electoralsearch.eci.gov.in/" target="_blank" className="hover:text-white transition-colors">Electoral Search</a></li>
              <li><a href="https://results.eci.gov.in/" target="_blank" className="hover:text-white transition-colors">Election Results</a></li>
            </ul>
          </div>
          <div>
      ...
      {showChat && (
        <div className="fixed inset-0 z-[100] bg-white animate-in slide-in-from-bottom-4 duration-500 overflow-hidden flex flex-col">
